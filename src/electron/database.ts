...
export class Database {
  ...
  getChats(offset: number, limit: number): Chat[] {
  ...
  getMessages(chatId: number, offset: number, limit: number): Message[] {
    const stmt = this.db.prepare(`
    ...
    return messages.map(msg => ({
      ...msg,
      body: securityService.decrypt(msg.body),
    }));
  }
  ...
  searchMessages(chatId: number, query: string): Message[] {
    ...
    const messages = stmt.all(chatId, `%${query}%`) as Message[];
  ...
  searchMessagesGlobal(query: string): Message[] {
    ...
    const messages = stmt.all(searchPattern, searchPattern, searchPattern) as Message[];
  ...
  addMessage(message: { chatId: number; ts: number; sender: string; body: string }): number {
    // Encrypt message body before storage
    // In production, this would use real encryption
    const encryptedBody = securityService.encrypt(message.body);
    ...
    const transaction = this.db.transaction(() => {
      ...
      console.log('[Database] Message added:', securityService.sanitizeForLogging(message));
  ...
  seedData(): void {
    ...
    if (chatCount.count > 0) {
      ...
      const legacyChats = this.db.prepare("SELECT id, title FROM chats WHERE title LIKE 'Chat %'").all() as { id: number, title: string }[];
    ...
    const transaction = this.db.transaction(() => {
      ...
      const chatLastMessageTime: Map<number, number> = new Map();
